...
SELECT country, COUNT(Movie_id)AS number_of_movies
FROM movies
GROUP BY country
ORDER BY number_of_movies DESC;

SELECT title_year, COUNT(Movie_id)AS number_of_movies
FROM movies
GROUP BY title_year
ORDER BY number_of_movies DESC;
...
SELECT movie_title,
   AVG(budget) OVER (ORDER BY title_year ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS avg_budget_of_last_3_years
FROM movies
...
SELECT d.directors,COUNT(*) AS movie_count , ROUND(AVG(imdb_score),2) as avg_imdb_score
FROM movies m
INNER JOIN directors d
ON m.Director_ID=d.D_ID
GROUP BY d.Directors
HAVING COUNT(*)>=3 AND ROUND(AVG(imdb_score)>7,2)
ORDER BY 3 DESC
